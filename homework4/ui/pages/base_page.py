...
class BasePage(object):
    ...
    def swipe_left_from_element(self, locator, count=1):
        web_element = self.find(locator, 10)
        dimension = self.driver.get_window_size()
        right_x = int(dimension['width'] * 0.9)
        left_x = int(dimension['width'] * 0.1)
        upper_y = web_element.location['y']
        lower_y = upper_y + web_element.rect['height']
        middle_y = (upper_y + lower_y) / 2
        action = TouchAction(self.driver)
    ...
    def search_element_by_text_use_scroll_left(self, locator, text):
        flag = True
        last_el = None
        while flag:
            ...
            for el in elements:
                if el.text == text:
                    return el
            if last_el and last_el == elements[-1].text:
                raise NoSuchElementException(f'Error, no such {text}')
            last_el = elements[-1].text
            self.swipe_left_from_element(locator)
